...
pub trait GpuDispatchable: Send + Sync {
    ...
    /// # Errors
    ///
    /// Returns [`CoreError::DeviceNotAvailable`] if the device type is not supported
    /// (e.g., Metal on non-Apple platforms). May also propagate errors from the
    /// underlying `dispatch_gpu` or `dispatch_cpu` implementations.
