...
fn bench_memory_tracker(c: &mut Criterion) {
    ...
    group.bench_function("would_fit", |b| {
        ...
        b.iter(|| {
            black_box(tracker.would_fit(black_box(400_000_000)));
        });
    });
    ...
    group.bench_function("estimate_with_overhead", |b| {
        ...
        let shape = [32, 64, 128];
        b.iter(|| {
            black_box(tracker.estimate_with_overhead(black_box(&shape), DType::F32));
        });
    });
...
fn bench_dtype_operations(c: &mut Criterion) {
    ...
    group.bench_function("is_half_precision", |b| {
        ...
        b.iter(|| {
            black_box(dtype.is_half_precision());
        });
    });
    ...
    group.bench_function("is_training_dtype", |b| {
        let dtype = DType::F32;
        b.iter(|| {
            black_box(dtype.is_training_dtype());
        });
    });
    ...
    group.bench_function("accumulator_dtype", |b| {
        let dtype = DType::BF16;
        b.iter(|| {
            black_box(dtype.accumulator_dtype());
        });
    });
